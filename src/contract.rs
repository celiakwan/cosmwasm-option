...
pub fn finalize(
    ...
    if info.funds != state.counter_offer {
        return Err(ContractError::CustomError {
            val: format!(
                "Counter offer mismatch. counter_offer: {:?}, funds: {:?}",
...
mod tests {
    ...
    fn test_instantiate() {
        ...
        assert_eq!(value.collateral, collateral);
        assert_eq!(value.counter_offer, counter_offer);
        assert_eq!(value.expires, 100_000);
    }
    ...
    fn test_transfer() {
        ...
        let err = transfer(deps.as_mut(), mock_env(), info, Addr::unchecked("anyone"))
            .unwrap_err();
        match err {
